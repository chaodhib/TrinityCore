...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    Unit* mover = client->unitBeingMoved;
    ...
    if (!client->IsInClientControlSet(guid))
    {
        TC_LOG_WARN("entities.unit", "!client->IsInClientControlSet(guid) for the client of player %s", _player->GetName());
        recvData.rfinish();                     // prevent warnings spam
        return;
    }
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPacket &recvData)
{
    ...
    if (client->unitBeingMoved != nullptr || !client->IsInClientControlSet(guid))
    {
    ...
    client->unitBeingMoved = ObjectAccessor::GetUnit(*_player, guid);
}
...
void WorldSession::HandleMoveNotActiveMover(WorldPacket &recvData)
{
    ...
    if (client->unitBeingMoved == nullptr || client->unitBeingMoved->GetGUID() != old_mover_guid)
    {
