...
enum Spells
{
    ...
    SPELL_DECIMATE                      = 28374, // 54426 in 25 man. Effect0 is handled by SpellScript (see below) and 2 rows in conditions table. Effect2 is handled by InstanceScript (see instance_naxxramas.cpp).
...
public:
    ...
    struct boss_gluthAI : public BossAI
    {
        ...
        boss_gluthAI(Creature* creature) : BossAI(creature, BOSS_GLUTH)
        {
            ...
            instance->SetData(DATA_GLUTH_ZOMBIES_RESET, true);
        }
        ...
        void Reset() override
        {
            ...
            state = STATE_GLUTH_NORMAL;
            instance->SetData(DATA_GLUTH_ZOMBIES_RESET, true); // reset the zombies GUID list in the Instance Script.
        }

        void EnterCombat(Unit* /*who*/) override
        {
            _EnterCombat();
            events.ScheduleEvent(EVENT_WOUND, 10 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_ENRAGE, urand(16, 22) * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_DECIMATE, urand(110, 120) * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_BERSERK, 8 * MINUTE*IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_SUMMON, 15 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_SEARCH_ZOMBIE_SINGLE, 5 * IN_MILLISECONDS);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_WOUND:
                        ...
                        {
                            events.ScheduleEvent(EVENT_WOUND, 3 * IN_MILLISECONDS);
                            break;
                        }
                    ...
                    case EVENT_ENRAGE:
                        if (state == STATE_GLUTH_EATING)
                        {
                            events.ScheduleEvent(EVENT_ENRAGE, 5 * IN_MILLISECONDS);
                            break;
                        }
                    ...
                    case EVENT_DECIMATE:
                        if (state == STATE_GLUTH_EATING)
                        {
                            events.ScheduleEvent(EVENT_DECIMATE, 4 * IN_MILLISECONDS);
                            break;
                        }
                        ...
                        for (int i = 1; i <= 20; i++)
                            events.ScheduleEvent(EVENT_SEARCH_ZOMBIE_MULTI, 3 * i * IN_MILLISECONDS);
                        events.ScheduleEvent(EVENT_DECIMATE, urand(110, 120) * IN_MILLISECONDS);
                        break;
                    case EVENT_BERSERK:
                        Talk(EMOTE_BERSERKER);
                        DoCast(me, SPELL_BERSERK);
                        events.ScheduleEvent(EVENT_BERSERK, 5 * MINUTE*IN_MILLISECONDS); //refresh the hard enrage buff
                        break;
                    case EVENT_SUMMON:
                        if (Is25ManRaid()) // one wave each 10s. one wave=1 zombie in 10man and 2 zombies in 25man.
                        {
                            DoSummon(NPC_ZOMBIE_CHOW, PosSummon[rand32() % 3]);
                            DoSummon(NPC_ZOMBIE_CHOW, PosSummon[rand32() % 3]);
                        }
                        else
                            DoSummon(NPC_ZOMBIE_CHOW, PosSummon[0]);
                        events.ScheduleEvent(EVENT_SUMMON, 10 * IN_MILLISECONDS);
                        break;
                    case EVENT_SEARCH_ZOMBIE_SINGLE:
                    {
                        ...
                        for (SummonList::const_iterator itr = summons.begin(); !zombie && itr != summons.end(); ++itr)
                            if (ObjectAccessor::GetCreature(*me, *itr)->IsWithinDistInMap(me, 10.0))
                        ...
                        if (zombie)
                        {
                            ...
                            zombie->AI()->SetData(DATA_ZOMBIE_STATE, STATE_ZOMBIE_TOBE_EATEN); // change the state of the zombie to STATE_ZOMBIE_TOBE_EATEN

                            // gluth should turn toward the zombie and stop AAs on his primary target. He then pauses for a few seconds
                            me->GetMotionMaster()->MoveFollow(zombie, 0, 0);
                            me->SetReactState(ReactStates::REACT_PASSIVE);
                            me->SetFacingToObject(zombie);
                            me->SetTarget(ObjectGuid::Empty);
                            state = STATE_GLUTH_EATING;

                            events.ScheduleEvent(EVENT_KILL_ZOMBIE_SINGLE, 2.5 * IN_MILLISECONDS);
                        }
                    ...
                    case EVENT_SEARCH_ZOMBIE_MULTI:
                    {
                        ...
                        {
                            zombie = ObjectAccessor::GetCreature(*me, *itr);
                            if (zombie->GetExactDist2d(me) > 18.0) // should maybe only affect "decimated" zombies  AFAIK. But I found no reliable evidence.
                                zombie = nullptr;
                        }
                        ...
                        if (zombie)
                        {
                            ...
                            DoCast(SPELL_ZOMBIE_CHOW_SEARCH_MULTI); // should maybe only affect "decimated" zombies  AFAIK. But I found no reliable evidence.
                        }
                        break;
                    }
                }
...
public:
    ...
    class spell_gluth_decimate_SpellScript : public SpellScript
    {
        ...
        // This method handles the damaging effect of the decimate spell.
    ...
    struct npc_zombie_chowAI : public ScriptedAI
    {
        ...
        void SetData(uint32 id, uint32 value) override
        {
            if (id == DATA_ZOMBIE_STATE) // change of state
            {
                ...
                {
                    // loose aggro behavior
                    me->SetReactState(ReactStates::REACT_PASSIVE);
                    me->AttackStop();
                    me->SetTarget(ObjectGuid::Empty);

                    // and forced to stand still
                    me->StopMoving();
